...
Options:
  ...
  -f file=../optimiz/auto93.csv  data file
  -h               show help
"""
...
def clone(data: o, rows: Optional[list[Row]] = None) -> o:
  return adds(rows or [], Data([data.cols.names]))

def adds(src: Iterator[Any], it: Optional[o] = None) -> o:
  it = it or Num()
  [add(it,x) for x in src]
  return it
def sub(x: o, v: Any, zap: bool = False) -> Any:
  return add(x,v,-1,zap)
def add(x: o, v: Any, inc: int = 1, zap: bool = False) -> Any:
  if v == "?": return v
  x.n += inc
...
def distKpp(data: o, rows: Optional[list[Row]] = None, k: int = 20, few: Optional[int] = None) -> list[Row]:
  few = few or the.Few
  ...
  while len(out) < k:
    ...
    for j, w in enumerate(ws):
      if (p := p - w) <= 0: 
          out += [tmp[j]]; break
  return out
def distProject(data: o, row: Row, east: Row, west: Row, c: Optional[float] = None) -> float:
  D = lambda r1,r2 : distx(data,r1,r2)
  c = c or D(east,west)  
  a,b = D(row,east), D(row,west)
  return (a*a +c*c - b*b)/(2*c + 1e-32)
def distFastmap(data: o, rows: Optional[list[Row]] = None) -> list[Row]:
  rows = rows or data.rows
...
def distFastermap(data: o, rows: Optional[list[Row]] = None, sway2: bool = True) -> list[Row]:
...
def like(i: o, v: Any, prior: float = 0) -> float:
  if i.it is Sym:
...
def likes(data: o, row: Row, nall: int = 100, nh: int = 2) -> float:
...
def likely(data: o, rows: Optional[list[Row]] = None) -> list[Row]:
  rows = rows or data.rows
...
def treeSelects(row: Row, op: str, at: int, y: Atom) -> bool:
  ...
  if op == "=="          : return x == y
  if op == ">"           : return x > y
def Tree(data: o, rows: Optional[list[Row]] = None, Y: Optional[Callable] = None, 
         Klass: Callable = Num, how: Optional[Op] = None) -> o:
  rows = rows or data.rows
  ...
  return tree
def treeCuts(col: o, rows: list[Row], Y: Callable, Klass: Callable) -> tuple[float, list[Op]]:
  xys = sorted([(r[col.at], Y(r)) for r in rows if r[col.at] != "?"])
  return (_symCuts if col.it is Sym else _numCuts)(col.at,xys,Y,Klass)
def _symCuts(at: int, xys: list[tuple[Any, Any]], Y: Callable, Klass: Callable) -> tuple[float, list[Op]]:
  d = {}
  for x, y in xys:
...
def _numCuts(at: int, xys: list[tuple[Any, Any]], Y: Callable, Klass: Callable) -> tuple[float, list[Op]]:
...
def treeLeaf(tree: o, row: Row) -> o:
  for kid in tree.kids:
    if treeSelects(row, *kid.how): return treeLeaf(kid, row)
  return tree
def treeNodes(tree: o, lvl: int = 0) -> Iterator[tuple[int, o]]:
  yield lvl, tree
  for kid in sorted(tree.kids, key=lambda kid: kid.mu):
    yield from treeNodes(kid, lvl + 1)
def treeShow(data: o, tree: o, win: Optional[Callable] = None) -> None:
